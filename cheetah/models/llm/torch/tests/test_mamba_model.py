...
if torch is not None:
    from cheetah.models.llm.torch.helpers import generate, load_safetensors
    from cheetah.models.llm.torch.mamba_model import MambaModel
...
ALPHA_F = 0.0
ALPHA_P = 0.0
MAX_NEW_TOKENS = int(os.getenv("TC_TEST_MAX_NEW_TOKENS", "4"))
...
class TestMambaModel(unittest.TestCase):
    def setUp(self):
        self.test_model = os.getenv(
            "TC_TEST_NEMOTRON_MODEL",
        ...
        if str(self.model_config.get("model_type", "")).lower() != "nemotron_h":
            self.skipTest(f"Model '{self.test_model}' is not a Nemotron-H checkpoint")
